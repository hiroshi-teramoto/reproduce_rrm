...
for ind, mol in enumerate(moleq):
    ...
    if ind < neq:
        if wchiral_sop:
            weqchiral.append('true')
            ip = '()'
        else:
            weqchiral.append('false')
            hom = GeneticOrderMatcher(mol,tol)
            tran = hom.match(invmol(mol))
            ip = 'PermList('+str([i+1 for i in tran[0][0]])+')'
            assert tran[0][3] <= tol
...
for i in range(len(gconns)):
    ...
    for j in [0,1]:
        ...
        if tran1[3] > tol and tran2[3] > tol:
            ...
            if len(tran1) == 0 and len(tran2) == 0:
                bom = BruteForceOrderMatcher(molrp[i][j]) 
                tran1 = bom.match(moleq[gconns[i][j]])
                tran2 = bom.match(moleq[inv[gconns[i][j]]])

                if tran1[3] > tol and tran2[3] > tol:
                    warnings.warn('{} : transition state, {} (0:reactant, 1:product), {} : tol'.format(i,j,min([tran1[3],tran2[3]])))
...
print("output GRRM graph as a dot file!!")
Ggrrm = nx.drawing.nx_agraph.to_agraph(G)
Ggrrm.write("GRRM_graph.dot")
...
for c in cycles:
    ...
    for e in c:
        if e[2]['perms'][0][0] == e[0] and e[2]['perms'][1][0] == e[1]:
            gp = gp + '*Inverse(PermList('+str([x.item() if hasattr(x,'item') else x for x in e[2]['perms'][0][1]])+'))'
            gp = gp + '*PermList('+str([x.item() if hasattr(x,'item') else x for x in e[2]['perms'][1][1]])+')'
        elif e[2]['perms'][0][0] == e[1] and e[2]['perms'][1][0] == e[0]:
            gp = gp + '*Inverse(PermList('+str([x.item() if hasattr(x,'item') else x for x in e[2]['perms'][1][1]])+'))'
            gp = gp + '*PermList('+str([x.item() if hasattr(x,'item') else x for x in e[2]['perms'][0][1]])+')'
        else:
...
with open(fglist,mode='w') as f:
    ...
    f.write('symc:=ClosureGroup(Group([')
    for gp in symc:
        f.write(gp+',')
    f.write(']), Concatenation([')
    for n in range(neq):
            f.write('GeneratorsOfGroup('+G.nodes[n]['group']+'),')
    for n in range(nts):
            f.write('GeneratorsOfGroup('+urt[n]+'),')
    f.write(']));\n')
    ...
    sorted_edges = sorted(G.edges(data=True), key=lambda x: x[2]['ind'])
    for e in sorted_edges:
        f.write('[['+str(e[2]['perms'][0][0]+1)+','+'PermList('+str([x.item() if hasattr(x,'item') else x for x in e[2]['perms'][0][1]])+')],')
        f.write('['+str(e[2]['perms'][1][0]+1)+','+'PermList('+str([x.item() if hasattr(x,'item') else x for x in e[2]['perms'][1][1]])+')]],')
    f.write('];\n')

    f.write('org_eq:='+str([x+1 for x in org_eq])+';\n')
    f.write('org_ts:='+str([x+1 for x in org_ts])+';\n')

    f.write("wchiral:={};\n".format(weqchiral).replace("'",""))
    f.write("sinv:={};\n".format(sinv).replace("'",""))
